...
// This module is a browser entry point. Keeping its DOM code behind this guard
// prevents accidental execution in Node-based tooling or server environments.
...
$('#crypto-form').addEventListener('submit',e=>{e.preventDefault();const data=$('#data-input').value.trim(), key=$('#key-input').value.trim(),error=$('#error');if(!/^[01]{16}$/.test(data)||!/^[01]{16}$/.test(key)){error.textContent='Plaintext/ciphertext dan key harus berisi tepat 16 karakter biner (0 atau 1).';return;}error.textContent='';const mode=$('input[name="mode"]:checked').value;render(mode==='encrypt'?Encrypt(parseInt(data,2),parseInt(key,2)):Decrypt(parseInt(data,2),parseInt(key,2)),mode);});
}
